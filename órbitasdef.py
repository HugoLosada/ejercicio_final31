...
Sun = sphere(pos=vector(0, 0, 0), radius=0.05, color=color.yellow,
             make_trail=True, interval=10)
Sun.mass = 1
Sun.v = vector(0, 0, 0)

Earth = sphere(pos=vector(1, 0, 0), radius=0.03, color=color.blue,
               make_trail=True, interval=10)
Earth.mass = 3.004e-6
Earth.v = vector(0, 6.283, 0)

Mars = sphere(pos=vector(rmarte * ca, rmarte * sa, 0), radius=0.03, color=color.red,
              make_trail=True, interval=10)
Mars.mass = mmarte
Mars.v = vector(vmarte * cca, vmarte * sca, 0)

Ship = sphere(pos=vector(rship, 0, 0), radius=0.01, color=color.orange,
              make_trail=True, interval=10)
Ship.mass = 3.214e-28
Ship.v = vector(0, vship, 0)

# se establece el valor de t como el paso de tiempo utilizado en la simulación. Este valor se utiliza más adelante en el bucle principal para actualizar las posiciones y velocidades de los cuerpos celestes.
dt = 2.73785078e-4

# imprime en la consola el valor del radio de influencia mínimo de Marte (Rmin). La impresión de este valor se realiza una vez al comienzo de la simulación para que el usuario pueda conocer este parámetro clave.
...
#Se abre el archivo posiciones.txt en modo de escritura ('w'). El uso de with garantiza que el archivo se cierre correctamente después de su uso.
with open('posiciones.txt', 'w') as file:
    # Inicia un bucle infinito que representa el tiempo continuo en la simulación.
    while True:
        # limita la velocidad de la simulación a 100 iteraciones por segundo.
        rate(100)
# Itera sobre la lista de cuerpos celestes, que incluye la Tierra (Earth), Marte (Mars), y la nave espacial (Ship).
        # Dentro del bucle, se verifica si el cuerpo actual es la nave espacial. Si es así, se realiza una simulación especial.
...
# Si la nave espacial está cerca de Marte y el indicador flag es 0 (no se ha ajustado la trayectoria), se realiza un ajuste especial de trayectoria
                else:
                    if flag == 0:
